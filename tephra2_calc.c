...
/* fprintf(log_file, "point[x][y]=%.0f, %.0f\n",new_xspace, new_yspace); */
  ...
  //jwhite - initialize online_mean counter
  stats->mean_falltime.count = 0.0;
     ...
     */           
     ...
     /* removed 2 lines, 2-22-2011
      ...
      average_windspeed_y * average_windspeed_y);
		...
		//jwhite online mean calc
		stats->mean_falltime.count++;
		if ((i == 0) && (j == 0))
		{
			stats->mean_falltime.mu_new = total_fall_time;
			stats->mean_falltime.mu_old = 0.0;
		}
		else
		{
			stats->mean_falltime.mu_new = stats->mean_falltime.mu_old + 
				(total_fall_time - stats->mean_falltime.mu_old) / stats->mean_falltime.count;
			stats->mean_falltime.mu_old = stats->mean_falltime.mu_new;
		}
...
#ifdef _PRINT  
//bug fix jwhite changed log_file to log
fprintf(log, "IN set_global_values ...");
#endif
